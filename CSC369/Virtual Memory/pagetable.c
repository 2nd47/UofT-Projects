 ...
 * pagetable entry for victim to indicate that virtual page is no longer int,
...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// Set PTE as invalid
		pte->frame &= ~(PG_VALID);
		// Swap PTE if it is dirty, otherwise it hasn't been modified
		// since it has been placed in the swap file
		if (pte->frame & PG_DIRTY) {
			evict_dirty_count += 1;
			pte->frame &= ~(PG_DIRTY);
			pte->frame |= PG_ONSWAP;
			// Page out and check for success
			if ((pte->swap_off = 
				swap_pageout(pte->frame >> PAGE_SHIFT, 
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *secpgtbl;
	unsigned idx = PGDIR_INDEX(vaddr); // get index into page directory

	// Use top-level page directory to get pointer to 2nd-level page table
	secpgtbl = (pgtbl_entry_t *)(pgdir[idx].pde);

	// If a first-level entry is invalid, we must create a second-level page table
	if (!((uintptr_t) secpgtbl & PG_VALID)) {
	...
	if (!(p->frame & PG_VALID)) {
		// Allocate space for the frame and set the frame number
		p->frame &= ~(PAGE_MASK);
		p->frame |= (allocate_frame(p) << PAGE_SHIFT);
		if (!(p->frame & PG_ONSWAP)) {
			// Not on swap, needs to be initialized
			init_frame(p->frame >> PAGE_SHIFT, vaddr);
			// Needs a chance to be swapped if it needs it
			p->frame |= PG_DIRTY;
		} else {
			// On swap, needs to be paged in
			// Check if paged in properly
			if (swap_pagein(p->frame >> PAGE_SHIFT, p->swap_off))
				exit(1);
		}
		// Regardless, allocate the frame since it is not in memory.
		miss_count += 1;
	} else {
		hit_count += 1;
	}
